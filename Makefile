...
build: clean vet main.go server device config go.mod go.sum
	...
	@echo "-> 4 Building the "${PROGRAM}_${GOOS}_armv8_64
	@GOOS=${GOOS} GOARCH=arm64 go build -trimpath -ldflags "-s -w" -o ./${OUTPUT}/${PROGRAM}_${GOOS}_armv8_64

	@echo "-> 5 Building the "${PROGRAM}_${GOOS}_386
	@GOOS=${GOOS} GOARCH=386 go build -trimpath -ldflags "-s -w" -o ./${OUTPUT}/${PROGRAM}_${GOOS}_386

	@echo "-> 6 Building the "${PROGRAM}_${GOOS}_amd64
